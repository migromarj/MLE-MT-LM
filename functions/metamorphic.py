...
async def distance_set_mrt(model, original_input, threshold=70):
    
    if (model[0] == 'fillmask'):
        ouput_1 = request_to_model(model, original_input)
        output_2 = request_to_bing(original_input, "fillmask")
        ...
        output_2 = request_to_bing(original_input, model[0])
...
async def calculate_M_ASR(model, disturbation_func, quality_attribute, change_meaning = False, iterations = 5):
    ...
    if (model[0]=='fillmask'):
        ...
        for _ in range(iterations):
            ...
            if (quality_attribute == 'Robustness'):
                ...
                fulfilled_relations += await distance_set_mrt(model, text)
                evaluated_relations += 1
            else:
    ...
    for text in texts:
        ...
        if model[0] == 'q&a' or model[0] == 'translate' or model[0] == 'fillmask' or model[0] == 'summarize':
            ...
            res_dict = distance_set_mrt(model, text)
